...
import org.smart4j.framework.mvc.bean.ApiBean;
...
public class DefaultViewResolver implements ViewResolver {
	...
	public void resolveView(HttpServletRequest request, HttpServletResponse response, Object actionMethodResult) {
		if (actionMethodResult != null) {
			...
			} else if (actionMethodResult instanceof ApiBean) {
				ApiBean api = (ApiBean) actionMethodResult;
				// 对于其它类型，统一转换为 JSON 格式并写入响应中
				WebUtil.writeJSON(response, api);
			} else {
